...
class EntityManager {
  constructor(world) {
    ...
    this._queryManager = new _QueryManager.default(world);
  ...
  entityAddArchetype(entity, Archetype, values) {
    ...
    if (Archetype._typeId === undefined && !this.world.archetypesManager._ArchetypesMap[Archetype._typeId]) {
      throw new Error(`Attempted to add unregistered Archetype "${Archetype.getName()}"`);
    }

    if (entity._Archetype) {
      if (process.env.NODE_ENV !== "production") {
        console.warn("Entity already has a archetype. Only 1 archetype can be added per entity.", entity, Archetype.getName());
      }
  ...
  entityAddComponent(entity, Component, values, isArchetype = false) {
    // @todo Probably define Component._typeId with a default value and avoid using typeof
    if (Component._typeId === undefined && !this.world.componentsManager._ComponentsMap[Component._typeId]) {
      throw new Error(`Attempted to add unregistered component "${Component.getName()}"`);
    }

    if (entity._components[Component._typeId]) {
      if (process.env.NODE_ENV !== "production") {
        console.warn("Component type already exists on entity.", entity._components.map(c => c.getName()), Component.getName());
      }
  ...
  entityRemoveAllComponents(entity, immediately) {
    for (const typeId of Object.keys(entity._components)) {
      ...
      if (Component.__proto__ !== _SystemStateComponent.SystemStateComponent) this.entityRemoveComponent(entity, Component, immediately);
    }
  }
  /**
