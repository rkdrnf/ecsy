...
class QueryManager {
  ...
  onEntityComponentAdded(entity, Component) {
    if (!this._componentQueryMap[Component._typeId]) return; // Check each indexed query to see if we need to add this entity to the list

    for (const query of this._componentQueryMap[Component._typeId]) {
      if (query.NotComponentsMask[Component._typeId] && ~query.entities.indexOf(entity)) {
      ...
      if (!query.ComponentsMask[Component._typeId] || !query.match(entity) || ~query.entities.indexOf(entity)) continue;
  ...
  onEntityComponentRemoved(entity, Component) {
    ...
    for (const query of this._componentQueryMap[Component._typeId]) {
      if (query.NotComponentsMask[Component._typeId] && !~query.entities.indexOf(entity) && query.match(entity)) {
        query.addEntity(entity);
        entity.queries.push(query);
        continue;
      }

      if (query.ComponentsMask[Component._typeId] && !!~query.entities.indexOf(entity) && !query.match(entity)) {
        query.removeEntity(entity);
        const index = entity.queries.indexOf(this);
        entity.queries.splice(index, 1);
        continue;
  ...
  getQuery(Components) {
    ...
    if (!query) {
      this._queries[key] = query = new _Query.default(Components, this._world.entityManager);

      const Archetypes = this._world.archetypesManager.Archetypes.filter(a => query.matchArchetype(a));
