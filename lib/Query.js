...
class Query {
  ...
  addEntity(entity, nocheck = false) {
    this.entities.push(entity);
    this.eventDispatcher.dispatchEvent(Query.prototype.ENTITY_ADDED, entity, null, {
      nocheck
    });
  }
  /**
  ...
  removeEntity(entity, nocheck = false) {
    ...
    if (~index) {
      ...
      this.eventDispatcher.dispatchEvent(Query.prototype.ENTITY_REMOVED, entity, null, {
        nocheck
      });
    }
  }
  ...
  match(entity) {
    return this.Components.every(c => entity._components[c._typeId]) && !this.NotComponents.some(c => entity._components[c._typeId]);
  }

  matchArchetype(Archetype) {
    return this.Components.every(c => Archetype._Components[c._typeId]) && !this.NotComponents.some(c => Archetype._Components[c._typeId]);
  }
