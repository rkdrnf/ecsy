export class ArchetypeManager {
  ...
  registerArchetype(Archetype) {
    if (this.Archetypes.indexOf(Archetype) !== -1) {
      console.warn(
        `Archetype type: '${Archetype.getName()}' already registered.`
      );
      return;
    }
    ...
    if (!schema) {
      throw new Error(
        `Archetype "${Archetype.getName()}" has no schema property.`
      );
    }
    ...
    for (const key of Object.keys(schema)) {
      ...
      if (!Component.isComponent) {
        throw new Error(
          `Invalid schema for archetype "${Archetype.getName()}". "${Component}" is not a component type.`
        );
      }

      if (Component._typeId === undefined) {
        throw new Error(
          `Invalid schema for archetype "${Archetype.getName()}". "${Component}" is not registered.`
        );
      }
